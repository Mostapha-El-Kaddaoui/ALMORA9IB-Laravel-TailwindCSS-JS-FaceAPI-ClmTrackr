...
<head>
    ...
    <script defer src="{{ asset('js/script-face-api.js') }}"></script>
    <script defer src="{{ asset('js/script-face-api.js') }}"></script>
...
<body>
    ...
    window.addEventListener('focus', function() {
        ...
        sendreq(comp_memory,endTime-startTime);
    });
    ...
    video.addEventListener('play', () => {
        ...
        setInterval(async () => {
            ...
            const leftEyeCenter = faceGeometry.getEyeCenter(leftEye);
            const rightEyeCenter = faceGeometry.getEyeCenter(rightEye);
            
            let horizantaleGazeDirection='';
            const Eyexcenter = (leftEyeCenter.x + rightEyeCenter.x) / 2;
            const Eyeycenter = (leftEyeCenter.y + rightEyeCenter.y) / 2;
            if(exam){
                sendreq(faceGeometry.getFaceHorizantaleDirection(Eyexcenter,nose[0].x)+' '+faceGeometry.getFaceVerticaleDirection(Eyeycenter,nose[0].y),0)
            }else{
    ...
    function sendreq(action_to_store,duration) {
        $.ajax({
            type: "POST",
            url: '/sendchange',
            data: { 
